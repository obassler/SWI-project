      ...
      <div className="space-y-6">
        ...
        <section className="grid grid-cols-2 gap-4">
                ...
                <ul className="space-y-1 mb-2">
                  {characters.map(char => (
                      <li key={char.id} className="bg-gray-600 p-1 rounded">
                        <Link to={`/characters/${char.id}`} className="block hover:bg-gray-500 p-1 rounded">
                          <span className="font-medium">{char.name}</span>
                          <span className="text-sm text-gray-300 ml-2">HP: {char.hp}</span>
                        </Link>
                      </li>
                  ))}
                </ul>
          ...
          <div className="bg-gray-700 p-4 rounded">
            ...
            {/* Action buttons */}
            <button className="mr-2 px-3 py-1 bg-yellow-600 rounded hover:bg-yellow-700">Roll Dice</button>
            <button className="px-3 py-1 bg-yellow-600 rounded hover:bg-yellow-700">Adjust HP</button>
          </div>
        </section>
      </div>
  );
